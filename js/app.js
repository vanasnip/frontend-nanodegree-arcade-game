...
Player.prototype = Object.create(Enemy.prototype); //inherit render functionality from Enemy
// Update the enemy's position, required method for game
// Parameter: dt, a time delta between ticks
...
Player.prototype.collision = function(){
  ...
  for(var j = 0; j < allGems.length; j++){
    ...
    if (allGems[j].x > this.x -80 && allGems[j].x < this.x + 80 && allGems[j].y === this.y){
      //resting the player position
      allGems[j].x = 5 + (45 * this.gemCollected); // creates offset of 45 units to the right of last gem drawn
...
Heart.prototype = Object.create(Gem.prototype); //inherit render functionality from Gem
